...
@section('content')
    ...
    <form method="post" name="question" action="{{action('PollResponseController@store')}}" accept-charset="UTF-8">
        ...
        @while(isset($question))
            ...
            <div class="question question_{{$question->id}}" data-title="{{$question->translation()->text}}" {!!$question->max_alternatives>1?'data-min-select="'.$question->min_alternatives.'"':''!!} data-id="{{$question->id}}" {!!empty($question->display_criteria)?"":'style=display:none data-display-criteria="'.$question->display_criteria.'"'!!}>
